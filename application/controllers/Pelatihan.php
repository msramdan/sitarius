...
class Pelatihan extends CI_Controller
{
	...
	public function add_peserta()
	{
		...
		if ($query > 0) {
			...
			$data_pelatihan = $this->db->query("SELECT * FROM pelatihan where pelatihan_id='$no_pelatihan_id'")->row_array();
			sendWa($rowPeserta->no_hp, 'add_peserta', $data_pelatihan);
	...
	public function create_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			foreach ($this->input->post('pemateri_id') as $value) {
				...
				$rowDbPemateri = $this->Pemateri_model->get_by_id($value);


				// add notif ramdan
				sendWa($rowDbPemateri->no_hp, 'add_pemateri', $data);
			}
	...
	public function uploadSertifikat()
	{
		...
		if ($this->upload->do_upload("photo")) {
			...
			$no_pelatihan_id = $this->input->post('pelatihan_id');
			$data_pelatihan = $this->db->query("SELECT * FROM pelatihan where pelatihan_id='$no_pelatihan_id'")->row_array();
			sendWa($rowPeserta->no_hp, 'uploadSertifikat', $data_pelatihan);
	...
	public function uploadTrf()
	{
		...
		if ($this->upload->do_upload("photo")) {
			...
			$no_pelatihan_id = $this->input->post('pelatihan_id');
			$data_pelatihan = $this->db->query("SELECT * FROM pelatihan where pelatihan_id='$no_pelatihan_id'")->row_array();
			sendWa($rowPeserta->no_hp, 'uploadTrf', $$data_pelatihan);
	...
	public function uploadbukti_honor()
	{
		...
		if ($this->upload->do_upload("photo")) {
			...
			$no_pelatihan_id = $this->input->post('pelatihan_id');
			$data_pelatihan = $this->db->query("SELECT * FROM pelatihan where pelatihan_id='$no_pelatihan_id'")->row_array();
			sendWa($rowPemateri->no_hp, 'uploadbukti_honor', $data_pelatihan);
